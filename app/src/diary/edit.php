<?php
    ...
    if ((string)filter_input( INPUT_SERVER, "REQUEST_METHOD" ) == "GET") {
        ...
        $oldImageName = $diary['image'];
        $comment = $diary['comment'];
        $releaseDay = $diary['release_date'];
        ...
        // 確認ボタン押下後の処理
        // パラメーターを取得
        $diaryId = (string)filter_input( INPUT_POST, "diaryId" );
        ...
        $releaseDay = (string)filter_input( INPUT_POST, "releaseDay" );
       ...
       if ($comment == "") {
        ...
        if (count($err_mesgs) === 0) {
            $dbh = connectDb();
            $sql = "update diaries set title = :title, path = :path, image = :image, comment = :comment, release_date = :releaseDay, modified = now() where id = :id";
            $stmt = $dbh->prepare($sql);
            $params = [
                ":id" => $diaryId,
                ":title" => $title,
    ...
    // 画像削除
    $image_delete_url = '/app/src/diary/delete.php?diaryId=' . $diaryId;
