<?php
    ...
    if ((string)filter_input( INPUT_SERVER, "REQUEST_METHOD" ) == "GET") {
        ...
        // 公開日
        if ($releaseDay < $today) {
            $err_mesgs[] = $errMsg['range']['releaseDay1'] . $today . $errMsg['range']['releaseDay2'];
        }
        // 画像
        ...
        if (count($err_mesgs) === 0) {
            $dbh = connectDb();
            $sql = "insert into diaries (user_id, title, path, image, comment, release_date, created, modified) values (:user_id, :title, :path, :image,  :comment, :release_date, now(), now())";
    ...
    // テンプレートを使用
    $template = $twig->load("diary/register.html");
    // レンダリング
